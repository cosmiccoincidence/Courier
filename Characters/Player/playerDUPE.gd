...
@export var zoom_max := 50.0  # Normal max zoom
...
func _physics_process(delta):
	...
	# Can only sprint if moving and have stamina (or god mode)
	if wants_to_sprint and is_moving and (current_stamina > 0 or god_mode):
		is_sprinting = true
		if not god_mode:
			use_stamina(sprint_stamina_cost * delta)
		time_since_sprint_stopped = 0.0
	else:
		is_sprinting = false  # IMPORTANT: Set to false when conditions not met
...
func _rotate_toward_mouse():
	...
	var new_angle = lerp_angle(current_angle, target_angle, rotation_speed * get_physics_process_delta_time())
...
func _input(event): # Toggle God Mode with L
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SEMICOLON:
		...
		# Find the current map and trigger its exit
